...
const processBuffer = (proxy, renderedBuffer, offlineAudioContext, options, processorDefinition) => tslib_1.__awaiter(this, void 0, void 0, function* () {
    const { length } = renderedBuffer;
    const numberOfInputChannels = options.channelCount * options.numberOfInputs;
    const numberOfOutputChannels = options.outputChannelCount.reduce((sum, value) => sum + value, 0);
    const processedBuffer = (numberOfOutputChannels === 0) ? null : offlineAudioContext.createBuffer(numberOfOutputChannels, length, renderedBuffer.sampleRate);
    if (processorDefinition === undefined) {
        throw new Error();
    }
    const audioNodeConnections = getAudioNodeConnections(proxy);
    const audioWorkletProcessor = yield getAudioWorkletProcessor(offlineAudioContext, proxy);
            ...
            render: (proxy, offlineAudioContext) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                if (nativeAudioWorkletNodeConstructor === null) {
                    ...
                    return Promise
                        ...
                        const processedBuffer = yield processBuffer(proxy, renderedBuffer, offlineAudioContext, options, processorDefinition);
                        if (processedBuffer !== null) {
                ...
                if (!isOwnedByContext(nativeNode, offlineAudioContext)) {
                    ...
                    for (const [nm, audioParam] of Array.from(proxy.parameters.entries())) {
                        yield renderAutomation(proxy.context, offlineAudioContext, audioParam, nativeNode.parameters.get(nm));
                    }
                }
                else {
                    // @todo Using Array.from() is a lazy fix that should not be necessary forever.
                    for (const [nm, audioParam] of Array.from(proxy.parameters.entries())) {
                        yield connectAudioParam(proxy.context, offlineAudioContext, audioParam, nativeNode.parameters.get(nm));
                    }
                }
